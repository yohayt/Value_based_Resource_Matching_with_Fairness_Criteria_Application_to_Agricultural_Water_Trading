...
class TradingSyntheticGraph(GraphData):
    def __init__(self, N=10,k=5,beta_high=0.7, beta_low=0.3,gamma=0.5, delta=0.5):
        ...
        buyers = [node for node, attributes in self.compatibility_graph.nodes(data=True) if attributes.get('bipartite') == 0]
        sellers = [node for node, attributes in self.compatibility_graph.nodes(data=True) if attributes.get('bipartite') == 1]
